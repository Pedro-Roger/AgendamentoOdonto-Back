...
function makeRepo() {
  return new MedicalRecordsRepository(mockPrisma as any);
}
...
describe('MedicalRecordsRepository — isolamento por tenant', () => {
  ...
  it('update lança NotFoundException quando count === 0', async () => {
    ...
    const repo = makeRepo();
    await expect(repo.update('r1', { content: {} }, 't1')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it('findAttachments escopa via medicalRecord.patient.tenantId', async () => {
    ...
    expect(mockPrisma.medicalRecordAttachment.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
