...
function makeService() {
    return new appointments_service_1.AppointmentsService(mockRepo, mockPatientAppointmentsService);
}
describe('AppointmentsService — AGD-001 week range', () => {
    ...
    it('throws when range exceeds 31 days', async () => {
        const svc = makeService();
        await expect(svc.findByDateRange('2026-06-01', '2026-07-10', 't1')).rejects.toThrow(common_1.BadRequestException);
    });
    it('throws when to is before from', async () => {
        const svc = makeService();
        await expect(svc.findByDateRange('2026-06-15', '2026-06-10', 't1')).rejects.toThrow(common_1.BadRequestException);
    });
